...
public class HttpResponseHandlerImpl implements HttpResponseHandler {
	...
	public boolean isHandleResponse(HttpLiarExchange exchange) {
		...
		if (!StringUtils.isEmpty(referUrl)
				&& (referUrl.contains("trace_id=") || referUrl
						.contains("filter_ids=")))
			return true;
		if (uri.contains("item.taobao.com")
				|| uri.contains("item.beta.taobao.com")) {
			return true;
		}
		return false;
	}
...
//		
//		final String jqueryJsonJS = IOUtils.toString(getClass().getResource("/javascript/jquery.json-2.4.min.js"));
//		final String jqueryPutin = IOUtils.toString(getClass().getResource("/javascript/jquery-put.js"));
//		final String jqueryCss = IOUtils.toString(getClass().getResource("/javascript/jquery-css.js"));
//		
//		if (null != bodyElement) {
//			bodyElement.append( "<script charset='utf-8'>" + jqueryPutin + "</script>" );
//			bodyElement.append("<script charset='utf-8'>" + jqueryJsonJS + "</script>");
//			bodyElement.append("<script charset='utf-8'>" + jqueryCss + "</script>");
//		}
//
//		return new ResponseHandlerResult(new HtmlBlock(doc.html(),
//				htmlBlock.getCharset()));
		String uri = exchange.getRequestURI();
		long comtId;
		String traceId = HttpFilterUtils.getTraceId(exchange.getRequestFields(), uri);
		SingleResultDO<List<CommunicationDO>> result = comtService
				.fetchComtByTraceId(traceId);
		if (result.isSuccess()) {
			...
			cont.setUrl(uri);
			SingleResultDO<ConnectionDO> contResult = contService
					.createConnectionDO(cont);
	...
	private RequestDO createReqDO(HttpLiarExchange exchange) throws IOException {
		...
		final Enumeration<String> reqHttpFieldEnum = reqHttpFields
				.getFieldNames();
		while (reqHttpFieldEnum.hasMoreElements()) {
			final String nameEnum = reqHttpFieldEnum.nextElement();
			final Enumeration<String> reqHttpFieldValueEnum = reqHttpFields
					.getValues(nameEnum);
	...
	private ResponseDO createResDO(HttpLiarExchange exchange, DataBlock block)
			throws IOException {
		...
		final Enumeration<String> resHttpFieldEnum = resHttpFields
				.getFieldNames();
		while (resHttpFieldEnum.hasMoreElements()) {
			final String nameEnum = resHttpFieldEnum.nextElement();
			final Enumeration<String> resHttpFieldValueEnum = resHttpFields
					.getValues(nameEnum);
			final List<String> values = new ArrayList<String>();
